...
- внутренние уведомления о новых заявках и назначенных задачах;
- Telegram-уведомления о заявках при подключённом боте;
- задачи менеджеров со сроками, ответственными и контролем просрочки;
- командные комментарии и история работы внутри заявки;
- клиенты, заявки, перевозки и история статусов;
- роли `admin`, `manager`, `logistician`;
- тарифы из админки управляют расчётом на сайте;
- официальная тарифная сетка из Алматы: за кг, м³ и крытую фуру 20 тонн;
- отдельные почасовые тарифы транспорта и спецтехники по Алматы;
- реквизиты ТОО и печатное коммерческое предложение с экспортом в PDF;
...
| `APP_TIMEZONE` | часовой пояс рабочих дат, по умолчанию `Asia/Almaty` |
| `TELEGRAM_BOT_TOKEN` | токен Telegram-бота для уведомлений, необязательно |
| `TELEGRAM_CHAT_ID` | ID чата или группы для уведомлений, необязательно |
...
Для Telegram создайте бота через BotFather, добавьте его в рабочую группу и укажите в Render секреты `TELEGRAM_BOT_TOKEN` и `TELEGRAM_CHAT_ID`. Без этих значений внутренние уведомления CRM продолжают работать в обычном режиме.
